...
class UsersModel extends CommonModel
{
    ...
    public function updateTokenById($user_id,$token){
        //return $this->execute("update ".$this->getTableName()." set token = '{$token}' where user_id = {$user_id}");
        $data = array(
            'token' => (string)$token
        );
        $this->where('user_id='.$user_id)->save($data);
    }
    public function updatePwByAccount($account,$Npassword){
        return $this->where('account='.$account)->save(array('password'=>md5($Npassword)));
    }
}