...
class AdrAction extends CommonAction {
    ...
    public function delete(){
            ...
            if(empty($addr_id)){
                $this->ajaxReturn(array('success'=>false,'error_msg'=>'地址不存在'));
            }
    ...
    public function add_addr(){
                ...
                if(!$name){
                    $this->ajaxReturn(array('success' => false, 'error_msg' => '联系人没有填写！'));
                }

                if(!$city_code || !$area_code || !$province_code){
                    $this->ajaxReturn(array('success' => false, 'error_msg' => '省区、城市、地区必须都选择！'));
                }

                if(!isMobile($mobile)){
                    $this->ajaxReturn(array('success' => false, 'error_msg' => '手机号码不正确！'));
                }

                if(!$addr){
                    $this->ajaxReturn(array('success' => false, 'error_msg' => '收货地址没有填写！'));
                }
                $ud = D('UserAddr');
                $data = array();
                $data['name'] = $name;
                ...
                $add = $ud -> add($data);
                if($add){
                    $this->ajaxReturn(array('success' => true, 'error_msg' => '添加成功！'));
                }else{
                    $this->ajaxReturn(array('success' => false, 'error_msg' => '添加失败！'));
                }
    ...
    public function edit_addr(){
                    ...
                    $default_add = $this->_post('default')?$this->_post('default'):0;
                    if(!$addr_id){
                        $this->ajaxReturn(array('success' => false, 'error_msg' => '错误！'));
                    }else{
                        ...
                        if(!$f){
                            ...
                            if($f['user_id'] != $this->app_uid){
                                $this->ajaxReturn(array('success' => false, 'error_msg' => '非法操作！'));
                            }
                        }
                    }
                    ...
                    if($add){
                        $this->ajaxReturn(array('success' => true, 'error_msg' => '修改成功！'));
                    }else{
                        $this->ajaxReturn(array('success' => false, 'error_msg' => '修改失败！'));
                    }
