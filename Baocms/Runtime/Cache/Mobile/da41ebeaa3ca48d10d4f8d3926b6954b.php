...
<html lang="zh-cn">
	...
	<header class="top-fixed bg-yellow bg-inverse">
		<div class="top-back">
			<a class="top-addr" href="<?php echo U('mart/index',array('cat'=>$detail['cate_id']));?>"><i class="icon-angle-left"></i></a>
		</div>
		<div class="top-title">
			订单设定
		</div>
...
.icon-angle-right{ float:right; font-size:20px;}
	...
	<form class="pay-form" action="<?php echo U('mall/pay2',array('order_id'=>$order['order_id']));?>" method="post"  target="x-frame">
        ...
        <span class="x10">
            ...
            <p onclick="location='<?php echo U("mcenter/addrs/index",array('type'=>3,'order_id'=>$order['order_id']));?>'"><?php echo ($citys[$item['city_id']]['name']); ?> <?php echo ($areas[$item['area_id']]['area_name']); ?> <?php echo ($business[$item['business_id']]['business_name']); ?> <?php echo ($item["addr"]); ?></p>
         </span>
          <span class="x2" onclick="location='<?php echo U("mcenter/addrs/index",array('type'=>3,'order_id'=>$order['order_id']));?>'"><i class="icon-angle-right padding-top"></i></span>
		</div>
