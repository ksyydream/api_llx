...
<div class="clear"></div>  
<?php $get_cat = $_GET['cat']; if(!$get_cat || empty($get_cat)){$get_cat = 1;} ?>
<div class="content"> 
    <div class="hdsy_banner">
        ...
        <div class="hdsy_flexslider">
            <ul class="slides">
                ...
                <?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Ad, bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=3,0,3,21600,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=3")->order("orderby asc")->limit("0,3")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="sy_hotgzLi flex-active-slide" style="width: 100%; float: left; margin-right: -100%; position: relative; opacity: 1; display: block; z-index: 2;"><a href="<?php echo ($item["link_url"]); ?>" title="<?php echo ($item["title"]); ?>" target="_blank">
                    
                    <img width="1200" height="272" src="<?php echo config_img($item['photo']);?>" draggable="false"></a></li> <?php endforeach; ?>
            </ul>
    ...
    <div class="hdsy_tab">
        <ul>
            <?php if(is_array($activitycates)): $index = 0; $__LIST__ = $activitycates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($index % 2 );++$index;?><li class="hdsy_tabLi <?php if($get_cat == $item['cate_id']){echo 'on';} ?>"><a href="<?php echo U('huodong/index',array('cat'=>$item['cate_id']));?>"><?php echo ($item["cate_name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    </div>
    ...
    <div class="hdsy_tabList">
        <ul>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="hdxq_tgList">
            <div class="left hdxq_tgList_l">
                <div class="hdxq_tjj">
                    <div class="hdsy_LiT"><?php echo ($item["title"]); ?></div>
                    <p class="hdsy_LiP"><?php echo ($item["intro"]); ?></p>
                </div>
                <div class="hdxq_ljct">
                <?php $num = D('Activitysign') -> where('activity_id ='.$item['activity_id']) -> sum(num); ?> 
                <?php echo ($num); ?>人已报名
                
                <?php if($item['sign_end'] < $today): ?><a href="<?php echo U('huodong/detail',array('activity_id'=>$item['activity_id']));?>" class="hdxq_ctAn" target="_blank">历史报名详情</a>
                <?php else: ?>
                 <a href="<?php echo U('huodong/detail',array('activity_id'=>$item['activity_id']));?>" class="hdxq_ctAn" target="_blank">立即报名</a><?php endif; ?>   
                
                
                </div>
                <div class="hdxq_LiTime">
                    有效期：至<?php echo ($item["sign_end"]); ?><a target="_blank" style="font-size:12px;" href="<?php echo U('shop/detail',array('shop_id'=>$item['shop_id']));?>">
                    商家：<?php echo ($shops[$item['shop_id']]['shop_name']); ?> <?php echo ($shops[$item['shop_id']]['tel']); if($shops[$item['shop_id']]['extension'] != null): ?>转<?php echo ($shops[$item['shop_id']]['extension']); ?>分机<?php endif; ?></a>
                </div>
            </div>
           <div class="right hdxq_tgList_r"><a href="<?php echo U('huodong/detail',array('activity_id'=>$item['activity_id']));?>"><img width="639" height="293" src="<?php echo config_img($item['photo']);?>"></a></div>
