...
<div class="content"> 
    <div class="hdsy_banner" style="height:auto">
        ...
        <div class="hdsy_flexslider">
            <ul class="slides">
                ...
                <?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Ad, bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=4,0,2,21600,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=4")->order("orderby asc")->limit("0,2")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="sy_hotgzLi flex-active-slide" style="width: 100%; float: left; margin-right: -100%; position: relative; opacity: 1; display: block; z-index: 2;"><a href="<?php echo ($item["link_url"]); ?>" title="<?php echo ($item["title"]); ?>" target="_blank">
                    <img width="1200" height="80" src="<?php echo config_img($item['photo']);?>" draggable="false"></a></li> <?php endforeach; ?>
            </ul>
    ...
    <div class="hdsy_tab">
        <ul>
        <li class="hdsy_tabLi on"><a href="<?php echo U('lifeservice/index');?>">全部家政</a></li>
            <?php if(is_array($lifeservicecates)): $index = 0; $__LIST__ = $lifeservicecates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($index % 2 );++$index;?><li class="hdsy_tabLi <?php if($get_cat == $item['cate_id']){echo 'on';} ?>"><a href="<?php echo U('lifeservice/index',array('cat'=>$item['cate_id']));?>"><?php echo ($item["cate_name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    </div>
        ...
        <ul>
            ...
            <?php if(is_array($list)): foreach($list as $key=>$item): ?><li class="yhsy_syLi">
                    <div class="yhsy_syLiNr">
                        <a href="<?php echo U('lifeservice/detail',array('id'=>$item['id']));?>" target="_blank" style="position:relative;">
                        	<img src="<?php echo config_img($item['photo']);?>" width="230"/>
                            <a href="<?php echo U('lifeservice/detail',array('id'=>$item['id']));?>" target="_blank"  class="dui-huan right" style="position:absolute;right:0;bottom:0;margin-bottom:23px;">预约</a>
                        </a>
                        <div class="left" style="height:85px;line-height:22px;">
                            <P style="height:44px;overflow:hidden;">姓名：<?php echo ($item["name"]); ?></P>
                            <p>服务价格：<?php echo round($item['price']/100,2);?>元/<?php echo ($item["unit"]); ?> </p>
                        </div>
                        
                        <div class="clear"></div>
                        <script type="text/javascript">
        $(function () {
            var str = "<?php echo ($host); echo U('moblie/lifeservice/detail',array('id'=>$item['id']));?>";
            $("#code_<?php echo ($item["id"]); ?>").empty();
            $('#code_<?php echo ($item["id"]); ?>').qrcode(str);
        })
                        </script>
                        <style>
                            .yhsy_syLiWx canvas{
                        ...
                        <div class="yhsy_syLiBgDw">
                            <div class="yhsy_syLiBgK">
                                <div class="yhsy_syLiDwNr">
                                    <a href="<?php echo U('lifeservice/detail',array('id'=>$item['id']));?>"><div class="yhsy_syLiWx"><div style="width: 122px; height: 122px; margin: 0px auto;" id="code_<?php echo ($item["id"]); ?>"></div></div>
                                        <p>[扫一扫，手机预约家政]</p></a>
                                </div>
