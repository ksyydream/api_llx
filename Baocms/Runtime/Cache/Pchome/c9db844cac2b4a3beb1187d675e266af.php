...
<div class="pagewd">
    <div class="seat_banner mt10">
        ...
        <div class="flexslider seat_flexslider">
            <ul class="slides">
                <?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Ad, bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=66,0,5,21600,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" bg_date <= '{$today}' AND  city_id IN ({$city_ids}) AND end_date >= '{$today}' AND closed=0 AND site_id=66")->order("orderby asc")->limit("0,5")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="list"><a target="_blank" href="<?php echo ($item["link_url"]); ?>"><img src="<?php echo config_img($item['photo']);?>" width="1200" height="200" /></a></li> <?php endforeach; ?>
            </ul>
        </div>
        <!--banner结束-->
    </div>
    ...
    <div class="tribeCont mt10">
    	<div class="left tribeCont_l">
            ...
            <?php if(is_array($cates)): foreach($cates as $key=>$item): if(($item["parent_id"]) == "0"): ?><h3><?php echo ($item["cate_name"]); ?></h3>
                <div>
                  <ul>
                   <?php if(is_array($cates)): foreach($cates as $key=>$item2): if(($item2["parent_id"]) == $item["cate_id"]): ?><li class="list"><a href="<?php echo U('news/index',array('cat'=>$item2['cate_id']));?>"><?php echo ($item2["cate_name"]); ?></a></li><?php endif; endforeach; endif; ?>  
                    </ul>
                    <div class="clear"></div>
                </div><?php endif; endforeach; endif; ?>    
            </div>
        ...
        <div class="right tribeCont_r">
            ...
            <div class="tribe_topic_box">
            	<div class="tribe_topic_tab">
                	<ul>
                     <li class="list <?php if($act == index): ?>on<?php endif; ?> "><a href="<?php echo U('news/index');?>">本地新闻</a></li>
            	    </ul>
                </div>
                <div class="tribe_topic_cont">
                    <div class="list_box mod-info-flow">
                    
                    <?php if(is_array($list)): foreach($list as $key=>$item): ?><div class="mod-b mod-art " data-aid="$item['article_id']">
                            <a href="<?php echo U('news/index',array('cat'=>$item['cate_id']));?>" class="column-link" ><?php echo ($cates[$item['cate_id']]['cate_name']); ?></a>
                               <div class="mod-thumb">
                                <a class="transition" href="<?php echo U('news/detail',array('article_id'=>$item['article_id']));?>" >
                                    <img alt="<?php echo ($item["title"]); ?>" src="<?php echo config_img($item['photo']);?>" style="display: inline;">
                                </a>
                            </div>
                            <div class="mob-ctt">
                                <h3><a href="<?php echo U('news/detail',array('article_id'=>$item['article_id']));?>" class="transition" ><?php echo ($item["title"]); ?></a></h3>
                                <div class="mob-author">
                                        <a href="<?php echo U('news/detail',array('article_id'=>$item['article_id']));?>">
                                            <span class="author-name ">作者：<?php echo ($item["source"]); ?></span><i class="i-icon icon-auth4" title="<?php echo ($item["source"]); ?>"></i>
                                        </a>
                                     <span class="time"><?php echo formatTime($item['create_time']);?></span>
                                    <i class="icon icon-cmt"></i><em><?php echo ($item["zan"]); ?></em>
                                    <i class="icon icon-fvr"></i><em><?php echo ($item["views"]); ?></em>
                                </div>
                                <div class="mob-sub"><?php echo bao_msubstr($item['profiles'],0,62,false);?></div>
                            </div>
                        </div><?php endforeach; endif; ?>    
