...
<div class="container">
		...
		<p><span>尊敬的：<?php echo ($SHOP["shop_name"]); ?>（<?php echo ($SHOP["shop_id"]); ?>）</span> 
               <?php if($counts['money'] > 0): ?>您总收入：<?php echo round($counts['money']/100,2);?>元，
               <?php $zong = $counts['money_day_goods_recipient'] + $counts['money_day_tuan_recipient'] + $counts['money_day_ele_recipient'] + $counts['money_day_ding_recipient']; ?>
               待确认收货：<?php echo round($zong/100,2);?>元<?php endif; ?>
...
<div class="blank-10 bg"></div>
	...
	<ul>
        ...
        <li><a>今日订座收入：<?php echo round($counts['money_day_ding']/100,2);?>元，昨日订座收入：<?php echo round($counts['money_day_ding_yesterday']/100,2);?>元。</a></li>
      	</ul>
	</div>
    <div class="blank-10 bg"></div>
    <div class="panel-list">
	<ul>
        <li><a>商城待确认收货：<?php echo round($counts['money_day_goods_recipient']/100,2);?>元。</a></li>
        <li><a>团购待确认收货：<?php echo round($counts['money_day_tuan_recipient']/100,2);?>元。</a></li>
        <li><a>外卖待确认收货：<?php echo round($counts['money_day_ele_recipient']/100,2);?>元。</a></li>
        <li><a>订座待确认收货：<?php echo round($counts['money_day_ding_recipient']/100,2);?>元。</a></li>
      	</ul>
	</div>
	...
	<ul>
        ...
        <li><a>商城（总）：<?php echo round($counts['money_goods']/100,2);?>元。</a></li>
        <li><a>订座（总）：<?php echo round($counts['money_ding']/100,2);?>元。</a></li>
      	</ul>
	</div>
